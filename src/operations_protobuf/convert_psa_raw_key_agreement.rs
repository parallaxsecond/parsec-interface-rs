...
use super::generated_ops::psa_raw_key_agreement::{
    Operation as OperationProto, Result as ResultProto,
};
use crate::operations::psa_raw_key_agreement::{Operation, Result};
...
impl TryFrom<OperationProto> for Operation {
    ...
    fn try_from(proto_op: OperationProto) -> std::result::Result<Self, Self::Error> {
        Ok(Operation {
            alg: convert_psa_algorithm::i32_to_raw_key_agreement(proto_op.alg)?,
            private_key_name: proto_op.private_key_name,
            peer_key: proto_op.peer_key.into(),
        })
    }
}
...
impl TryFrom<Operation> for OperationProto {
    ...
    fn try_from(op: Operation) -> std::result::Result<Self, Self::Error> {
        Ok(OperationProto {
            alg: convert_psa_algorithm::raw_key_agreement_to_i32(op.alg),
            private_key_name: op.private_key_name,
            peer_key: op.peer_key.to_vec(),
        })
    }
}
...
impl TryFrom<ResultProto> for Result {
    ...
    fn try_from(proto_op: ResultProto) -> std::result::Result<Self, Self::Error> {
        Ok(Result {
            shared_secret: Secret::new(proto_op.shared_secret),
        })
    }
}
...
impl TryFrom<Result> for ResultProto {
    ...
    fn try_from(op: Result) -> std::result::Result<Self, Self::Error> {
        Ok(ResultProto {
            shared_secret: op.shared_secret.expose_secret().to_vec(),
        })
    }
}
...
mod test {
    ...
    #[test]
    fn psa_raw_key_agreement_op_from_proto() {
    ...
    #[test]
    fn psa_raw_key_agreement_op_to_proto() {
        let name = "test name".to_string();
        let key_data = vec![0x11, 0x22, 0x33];
        let op = Operation {
            key_name: name.clone(),
